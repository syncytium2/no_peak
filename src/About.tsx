  ...
  return (
    <div className="about">
      ...
      <p className="note">
        ...
        random number generator and correspond to no animal, experiment, or measurement. Real
        laboratory recordings are not distributed with this app. Their <em>scales</em>, though —
        sampling interval, baseline, pulse amplitude, pulse frequency, pulse shape, assay CV — are
        taken from the published literature rather than chosen to look right, because a demo
        dataset on the wrong time scale teaches the wrong window widths. Each carries a note saying
        what it is meant to resemble, everything from the menu is tagged <em>simulated</em> next to
        its name, and the flat control is included deliberately: a detector should find nothing in
        it.
      </p>
      <p className="note">
        The two hormones are generated by different models on purpose. Portal GnRH is collected as
        continuous fractions and cleared far faster than any collection window, so its pulses are
        square bursts that land in one or two samples and return abruptly — there is no decay tail
        to model. Peripheral LH is a concentration with a half-life long compared with the sampling
        interval, so exponential clearance is right there and each pulse spans several samples.
        That is why the two look so unalike. The sources for every scale are listed in{" "}
